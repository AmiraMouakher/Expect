...
prediction_results = pd.DataFrame(columns=['meter_Id','Actual_daily','Predicted_daily'],index=range(len(datas)*16))
...
#     # reshape input to be [samples, time steps, features]
    ...
    print(model.summary())
    model.fit(x = [trainX,trainW, trainF], y= trainY,  epochs=100,verbose=1)
...
#     testY = scaler.inverse_transform(testY.ravel().reshape(-1, 1))
    ...
    test_errors_lstm.to_csv('Errors_weekly_T2V_W2V.csv',index=False)
    prediction_results.to_csv('PredVsActual_weekly_T2V_W2V.csv',index=False)
