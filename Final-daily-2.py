...
c = pd.read_csv('Data/data/Train.csv')
# w_avg = pd.read_csv('Data/data/weather-avg.csv')
info = pd.read_csv('Data/data/addinfoWissemDistance.csv')
...
def create_dataset(dataset, look_back):
    ...
    while i < len(dataset)-look_back :
        ...
        dataX.append(a)
        b = dataset[j:(j+look_back)]['consommation']
...
prediction_results = pd.DataFrame(columns=['meter_Id','Actual_daily','Predicted_daily'],index=range(len(datas)*90))
...
#     dataset = scaler.fit_transform(dataset)
    ...
    trainX, trainF, trainW, trainY = create_dataset(train, look_back)
    testX,testF, testW, testY = create_dataset(test, look_back)
...
#     # reshape input to be [samples, time steps, features]
    ...
    print(model.summary())
    model.fit(x = [trainX,trainW, trainF], y= trainY,  epochs=50,verbose=1)
...
#     testY = scaler.inverse_transform(testY.ravel().reshape(-1, 1))
    ...
    test_errors_lstm.to_csv('Errors_daily_T2V_W2V.csv',index=False)
    prediction_results.to_csv('PredVsActual_daily_T2V_W2V.csv',index=False)
